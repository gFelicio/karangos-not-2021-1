...
export default function KarangosForm() {
    ...
    function validate(data) {
        ...
        if (data.marca.trim() === '') {
            errorTemp.marca = 'Campo MARCA deve ser preenchido'
            isValid = false
        }

        if (data.modelo.trim() === '') {
            errorTemp.modelo = 'Campo MODELO deve ser preenchido'
            isValid = false
        }

        if (data.cor.trim === '') {
            errorTemp.cor = 'Campo COR deve ser preenchido'
            isValid = false
        }

        if (data.placa.trim() === '' || data.placa.includes('_')) {
            errorTemp.placa = 'Campo PLACA deve ser preenchido corretamente'
            isValid = false
        }
        
        if (isNaN(data.preco) || Number(data.preco) <= 0) {
            errorTemp.preco = 'Campo PREÇO deve ser preenchido com um valor positivo'
            isValid = false
        }
